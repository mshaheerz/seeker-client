...
import { deletePost, unLike, postLike, Notify } from "@/config/endpoints";
...
function Posts({ post, postPage }: any) {
  ...
  const refreshcomment = useSelector(
  ...
  useEffect(() => {
    async function fetchlike() {
      const data = await fetchLikes(post._id, {
        usertoken: localStorage.getItem("usertoken"),
      });
  ...
  const likePost = async () => {
    if (liked) {
      ...
      await Notify({
        ...
        content:`${users?.firstname} ${users?.lastname} liked your post`,
        href:`/${post?._id}`
      })
      setSendNotification({recieverId:post?.user?._id,notification:`${users?.firstname} ${users?.lastname} is liked your post`})
      setLiked(true);
      console.log(data);
    }
  };
